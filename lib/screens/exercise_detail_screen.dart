...
class _ExerciseDetailScreenState extends State<ExerciseDetailScreen> {
  ...
  void _navigateTo(int index) {
    if (index < 0 || index >= widget.allExercises.length) return;
    final provider = context.read<WorkoutProvider>();
    final current = provider.activeSession?.exercises.firstWhere(
      (e) => e.id == widget.sessionExercise.id,
      orElse: () => widget.sessionExercise,
    ) ?? widget.sessionExercise;
    if (current.sets.isNotEmpty && !current.completed) {
      provider.markExerciseCompleted(widget.sessionExercise.id!);
    }
      ...
      PageRouteBuilder(
        pageBuilder: (_, animation, __) => FadeTransition(
          ...
          child: ExerciseDetailScreen(
            sessionExercise: widget.allExercises[index],
  ...
  void _goToPrevious() => _navigateTo(widget.currentIndex - 1);
  void _goToNext() => _navigateTo(widget.currentIndex + 1);
  ...
  Future<void> _addSet() async {
    ...
    if (_historicPR == null || weight > (_historicPR!)) {
      _historicPR = weight;
      _showPRBadge();
    } else {
      // stesso peso o inferiore: nascondi subito il badge se era visibile
      _prTimer?.cancel();
      setState(() => _prBadgeOpacity = 0.0);
    }
    _recalcPR([...currentSets,
      ExerciseSet(sessionExerciseId: se.id!, setNumber: 0, weight: weight, reps: reps)]);

    if (context.read<ThemeProvider>().autoStartTimer) {
      _startTimer();
    }
  }
        ...
        return Scaffold(
                ...
                child: Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 4),
                      Text('PR!', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ),
              if (widget.currentIndex < widget.allExercises.length - 1)
                IconButton(
                  iconSize: 30,
          ...
          body: GestureDetector(
            onHorizontalDragEnd: (details) {
              ...
              if (details.primaryVelocity! > 300) _goToPrevious();
            },
            child: ListView(
